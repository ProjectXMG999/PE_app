...
1. **Co mógłbym powiedzieć o sobie** z tym słowem?
2. **Co mógłbym powiedzieć do bliskiej osoby?**
3. **Co mógłbym powiedzieć w pracy, w domu albo w trakcie zwykłego dnia?**
        ...
        <div className="training-detail">
          ...
          <div className="training-detail__content">
            {selectedExercise.fullDescription.split('\n').map((line, idx) => {
              if (line.startsWith('##')) {
                return <h2 key={idx} className="training-detail__h2">{line.replace(/^##\s*/, '')}</h2>
              }
              if (line.startsWith('###')) {
                return <h3 key={idx} className="training-detail__h3">{line.replace(/^###\s*/, '')}</h3>
              }
              if (line.startsWith('-')) {
                return <li key={idx} className="training-detail__li">{line.replace(/^-\s*/, '')}</li>
              }
              if (line.trim() === '') {
