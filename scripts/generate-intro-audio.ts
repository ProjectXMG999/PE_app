...
const WELCOME_PARAGRAPHS = [
  'Witaj w Language Performance Training.',
  'To jest Twój profesjonalny językowy trening. Zaprojektowaliśmy go tak, żeby prowadzić Cię krok po kroku — jak trener na siłowni.',
  'Naszym celem jest prosta rzecz: chcemy nauczyć Cię tych słów, które dadzą Ci najwięcej mówienia w najkrótszym czasie. Dlatego słownictwo nie jest tutaj przypadkowe. Jest ułożone od najczęściej używanych słów do coraz bardziej precyzyjnych. Najpierw uczysz się tego, co naprawdę pozwala przetrwać, potem dogadać się, potem zareagować w każdej sytuacji i wreszcie te sytuacje tworzyć.',
