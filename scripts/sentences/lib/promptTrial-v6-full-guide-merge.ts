...
// ============================================================================
// TRIAL PROMPT v8 — full merge of the owner's original "Project English
// Sentence Style Guide" (Manifest, 3-layer PASS/FAIL, Sentence Quality
// Score, Three Pillars, 22-item Red List, Golden Rule) with every fix
// discovered through live A/B testing in this pipeline (v1-v7, archived as
// promptTrial-v1-en-first.ts, promptTrial-v3-pl-first-fixed.ts,
// promptTrial-v4-anti-kitsch.ts, promptTrial-v5-polysemy-fix.ts).
// ----------------------------------------------------------------------------
// Why a merge, not a concatenation: the two documents together would be
// long enough to measurably dilute rule adherence — this pipeline already
// has direct evidence of that happening (the inflection-permission rule for
// "Lubiący" worked once, then silently regressed two revisions later after
// more rules were added, with no change to its own wording). So this
// version is organized in TIERS by mechanism and failure cost, with
// overlapping Red List items absorbed into named gates rather than stated
// twice, and the highest-failure-rate checks repeated once at the point of
// writing and once in a compressed final checklist (the pattern that
// empirically kept the inflection rule alive across the polysemy-fix
// round).
//
// Reconciled tension: the original guide says Emotional Memory is a bonus
// quality at 30-40% of the WHOLE BASE, not mandatory per sentence.
// Production prompt.ts independently made it mandatory-per-sentence turns
// ago to fix a real "flat sentences" complaint, and that fix was never
// walked back. Kept mandatory here — the 5-slot structure (candidate2/4)
// mechanically guarantees emotional flavor in 2/5 slots already exceeding
// the original 30-40% target, but slots 1/3/5 are NOT emotion-slots by
// definition, so a per-sentence floor is a different, still-needed
// guarantee, not a duplicate. "Mandatory" means no sentence may have ZERO
// emotional charge — a quiet trace (candidate5's plainness) still counts,
// per the guide's own resolution of this exact question.
// ============================================================================

export const SYSTEM_PROMPT_TRIAL = `Jesteś redaktorem zdań przykładowych dla Project English — aplikacji do nauki angielskiego dla Polaków.
...
RÓŻNORODNOŚĆ WEWNĄTRZ JEDNEGO SŁOWA: candidate1-5 TEGO SAMEGO słowa nie mogą dzielić tego samego początku zdania ani szkieletu składniowego. Zła sytuacja: kilka kandydatur zaczyna się identycznie, np. "Wchodzę w to, żeby [...]." powielone kilka razy. Porównaj pierwsze 3-4 słowa i strukturę — jeśli dwie się pokrywają, przepisz jedną od zera.
...
1. Czy polskie słowo bazowe zostało NATURALNIE odmienione, a nie wklejone jako sztywna forma słownikowa? ("Jestem/jesteś/jest + imiesłów" to prawie zawsze błąd — zamień na czasownik.)
...
5. Czy ta kandydatura dzieli szkielet zdania z inną kandydaturą tego samego słowa?
